<?php
    if (!class_exists('cl_core')) {
                            ...
                            if ( !empty( $posts ) ) {
                                ...
                                for($j = 0; $j < $k; $j++) {
                                    if (isset($post_replace[$j]) && $post_id != $post_replace[$j]->ID) {
                                        ...
                                        switch($words_plus) {
                                            case 0 :
                                                $links_replace[$links[$j]] = preg_replace( "/({$link})/u", "<a href=\"" . get_permalink($post_replace[$j]->ID)  . "\" title=\"$0\" alt=\"$0\">$0</a>", $links[$j] );
                                                break;
                                            case 1 :
                                                ...
                                                if (!in_array($first_word, self::$black_words) && !preg_match("/(\(|\)|\.|\,|\?|\!|\:|\;)/", "{$first_word} {$word}")) {
                                                    $links_replace[$links[$j]] = "<a href=\"" . get_permalink($post_replace[$j]->ID)  . "\" title=\"{$first_word} {$word}\" alt=\"{$first_word} {$word}\">{$first_word} {$word}" . "</a> " . $last_word;
                                                } else {
                                                    if (!in_array($last_word, self::$black_words) && !preg_match("/(\(|\)|\.|\,|\?|\!|\:|\;)/", "{$word} {$last_word}") ) {
                                                        $links_replace[$links[$j]] = "{$first_word} <a href=\"" . get_permalink($post_replace[$j]->ID)  . "\" title=\"{$word} {$last_word}\" alt=\"{$word} {$last_word}\">{$word} {$last_word}</a>";
                                                    } else {
                                                        $links_replace[$links[$j]] = preg_replace( "/({$link})/u", "<a href=\"" . get_permalink($post_replace[$j]->ID) . "\" title=\"$0\" alt=\"$0\">$0</a>", $links[$j] );
                                                    }
                                                }
                                            ...
                                            case 2 :
                                                ...
                                                if (!in_array($last_word, self::$black_words) && !preg_match("/(\(|\)|\.|\,|\?|\!|\:|\;)/", "{$word} {$last_word}") ) {
                                                    ...
                                                    if (isset($words[0]) && !in_array($words[0], self::$black_words) && !preg_match("/(\(|\)|\.|\,|\?|\!|\:|\;)/", "{$words[0]} {$words[1]}")) {
                                                        $links_replace[$links[$j]] = "<a href=\"" . get_permalink($post_replace[$j]->ID) . "\" title=\"{$first_word} {$word}\" alt=\"{$first_word} {$word}\">{$first_word} {$word}" . "</a> " . $last_word;
                                                    } else {
                                                        $links_replace[$links[$j]] = preg_replace( "/({$link})/u", "<a href=\"" . get_permalink($post_replace[$j]->ID) . "\" title=\"$0\" alt=\"$0\">$0</a>", $links[$j] );
                                                    }
        ...
        static function add_linking_text($cat_id, $linking_text = '')
        {
            ...
            if (!empty($cat_id) && !empty($linking_text) ) {
